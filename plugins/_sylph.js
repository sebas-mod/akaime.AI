    ...
    try {
        if (m.isGroup && !chat.isBanned) {
            ...
            if (!conn.sylph[m.sender]) {
                conn.sylph[m.sender] = [{
                    role: 'system',
                    content: `Eres Akame, una inteligencia artificial creada por KenisawaDev (también conocido como "Keni" o "Mauro"). Responde de manera clara y concisa. El nombre del usuario será: ${conn.getName(m.sender)}`,
                }];
            }
           if (conn.sylph[m.sender].length > 10) {
            ...
            if (botMentioned) {
                ...
                let match = m.text.match(/\b(Akame|akame)\b[\s]*([\s\S]*)/i);
                if (match) {
